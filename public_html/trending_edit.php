...
/**
 * Isolated Trending Articles Edit Image Optimizer
 ...
 */
function optimize_trending_edit_image($sourcePath, $maxWidth = 1600, $maxHeight = 1200, $quality = 82) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_submit'])) { 
    if (!validate_csrf_token($_POST['csrf_token'] ?? '')) {
        creed_audit_log('CSRF_REJECTED', 'TRENDING', $id, 'FAILURE');
    ...
    if (!empty($_FILES['image']['tmp_name'])) {
        ...
        if (!$uploadResult['success']) {
            ...
            creed_audit_log('UPLOAD_REJECTED', 'TRENDING', $id, 'FAILURE', ['error' => $uploadResult['error']]);
        } else {
            $newImageName = $uploadResult['filename'];

            // Attempt isolated Trending Edit WebP auto-optimization
            $optimizedResult = optimize_trending_edit_image($uploadResult['filepath'], 1600, 1200, 82);
            if (
                $optimizedResult['success'] &&
                !empty($optimizedResult['filename']) &&
                !empty($optimizedResult['filepath']) &&
                file_exists($optimizedResult['filepath']) &&
                !is_dir($optimizedResult['filepath']) &&
                ...
                creed_audit_log('UPLOAD_ACCEPTED', 'TRENDING', $id, 'SUCCESS', [
    ...
    if (empty($error)) {
        ...
        if ($connect instanceof mysqli) {
            if ($newImageName !== null) {
                $deleteimage = '';
                $stmtOld = mysqli_prepare($connect, "SELECT `blog_image` FROM `trending` WHERE `id` = ? LIMIT 1");
                if ($stmtOld) {
                ...
                $stmtUp = mysqli_prepare($connect, "UPDATE `trending` SET `blog_image` = ?, `title` = ?, `detail` = ? WHERE `id` = ?");
                if ($stmtUp) {
                    mysqli_stmt_bind_param($stmtUp, "sssi", $newImageName, $title, $detail, $id);
                    if (mysqli_stmt_execute($stmtUp)) {
                        creed_audit_log('UPDATE', 'TRENDING', $id, 'SUCCESS');
                        ...
                        creed_audit_log('UPDATE_FAILED', 'TRENDING', $id, 'FAILURE', ['error' => mysqli_stmt_error($stmtUp)]);
                    }
                    ...
                    creed_audit_log('UPDATE_FAILED', 'TRENDING', $id, 'FAILURE', ['error' => mysqli_error($connect)]);
                }
            } else {
                $stmtUp = mysqli_prepare($connect, "UPDATE `trending` SET `title` = ?, `detail` = ? WHERE `id` = ?");
                if ($stmtUp) {
        ...
        header("Location: trending.php?action=saved");
        exit;
    }
}
...
if ($connect instanceof mysqli && $id > 0) {
    $stmtFetch = mysqli_prepare($connect, "SELECT * FROM `trending` WHERE `id` = ? LIMIT 1");
    if ($stmtFetch) {
...
<head>
  ...
  <title>Edit Trending | Creed Tech Admin</title>
...
<body style="background:#F8FAFC;">
  ...
  <section style="margin-top: 100px;max-width:800px;margin-left:auto;margin-right:auto;padding:24px;background:#fff;border-radius:8px;border:1px solid #E2E8F0;">
    <h2 style="font-size:1.5rem;font-weight:700;color:#0F172A;margin-bottom:20px;">Edit Trending Item #<?php echo e($id); ?></h2>
    <?php if (!empty($error)) : ?>
    ...
    <?php endif; ?>
    <form action="trending_edit.php?id=<?php echo e($id); ?>" method="POST" enctype="multipart/form-data">
      <?php echo csrf_field(); ?>
      <div class="mb-3">
      ...
      <button class="btn btn-primary" type="submit" name="update_submit">Update Trending</button>
      <a href="trending.php" class="btn btn-secondary ms-2">Cancel</a>
    </form>
  </section>
</body>
</html>
