<?php
/**
 * Creed Tech - Administrative Article Delete Handler
 */
...
if (!validate_csrf_token($_POST['csrf_token'] ?? '')) {
    creed_audit_log('CSRF_REJECTED', 'ARTICLE', $_POST['id'] ?? null, 'FAILURE');
...
if ($id && $connect instanceof mysqli) {
    ...
    $stmtDel = mysqli_prepare($connect, "DELETE FROM `article` WHERE `id` = ?");
    if ($stmtDel) {
        mysqli_stmt_bind_param($stmtDel, "i", $id);
        mysqli_stmt_execute($stmtDel);
        mysqli_stmt_close($stmtDel);
        creed_audit_log('DELETE', 'ARTICLE', $id, 'SUCCESS');
    }
}

header("Location: article.php?deleted=1");
exit;