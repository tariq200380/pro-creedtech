<?php
/**
 * Creed Tech - Administrative Events Delete Handler
 */
...
if (!validate_csrf_token($_POST['csrf_token'] ?? '')) {
    creed_audit_log('CSRF_REJECTED', 'EVENT', $_POST['id'] ?? null, 'FAILURE');
...
if ($id && $connect instanceof mysqli) {
    ...
    $stmt = mysqli_prepare($connect, "SELECT `blog_image` FROM `events` WHERE `id` = ? LIMIT 1");
    if ($stmt) {
    ...
    $stmtDel = mysqli_prepare($connect, "DELETE FROM `events` WHERE `id` = ?");
    if ($stmtDel) {
        mysqli_stmt_bind_param($stmtDel, "i", $id);
        mysqli_stmt_execute($stmtDel);
        mysqli_stmt_close($stmtDel);
        creed_audit_log('DELETE', 'EVENT', $id, 'SUCCESS');
    }
}
