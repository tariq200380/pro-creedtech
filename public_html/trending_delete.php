<?php
/**
 * Creed Tech - Administrative Trending Delete Handler
 */
...
if (!validate_csrf_token($_POST['csrf_token'] ?? '')) {
    creed_audit_log('CSRF_REJECTED', 'TRENDING', $_POST['id'] ?? null, 'FAILURE');
...
if ($id && $connect instanceof mysqli) {
    ...
    $stmt = mysqli_prepare($connect, "SELECT `blog_image` FROM `trending` WHERE `id` = ? LIMIT 1");
    if ($stmt) {
    ...
    $stmtDel = mysqli_prepare($connect, "DELETE FROM `trending` WHERE `id` = ?");
    if ($stmtDel) {
        mysqli_stmt_bind_param($stmtDel, "i", $id);
        mysqli_stmt_execute($stmtDel);
        mysqli_stmt_close($stmtDel);
        creed_audit_log('DELETE', 'TRENDING', $id, 'SUCCESS');
    }
}

header("Location: trending.php?deleted=1");
exit;