<?php
/**
 * Creed Tech - Administrative Stories Delete Handler
 */
...
if (!validate_csrf_token($_POST['csrf_token'] ?? '')) {
    creed_audit_log('CSRF_REJECTED', 'STORY', $_POST['id'] ?? null, 'FAILURE');
...
if ($id && $connect instanceof mysqli) {
    ...
    $stmt = mysqli_prepare($connect, "SELECT `blog_image` FROM `stories` WHERE `id` = ? LIMIT 1");
    if ($stmt) {
    ...
    $stmtDel = mysqli_prepare($connect, "DELETE FROM `stories` WHERE `id` = ?");
    if ($stmtDel) {
        mysqli_stmt_bind_param($stmtDel, "i", $id);
        mysqli_stmt_execute($stmtDel);
        mysqli_stmt_close($stmtDel);
        creed_audit_log('DELETE', 'STORY', $id, 'SUCCESS');
    }
}

header("Location: stories.php?deleted=1");
exit;