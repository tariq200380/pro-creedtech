...
/**
 * Isolated Events Edit Image Optimizer
 ...
 */
function optimize_events_edit_image($sourcePath, $maxWidth = 1600, $maxHeight = 1200, $quality = 82) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_submit'])) { 
    if (!validate_csrf_token($_POST['csrf_token'] ?? '')) {
        creed_audit_log('CSRF_REJECTED', 'EVENT', $id, 'FAILURE');
    ...
    if (!empty($_FILES['image']['tmp_name'])) {
        ...
        if (!$uploadResult['success']) {
            ...
            creed_audit_log('UPLOAD_REJECTED', 'EVENT', $id, 'FAILURE', ['error' => $uploadResult['error']]);
        } else {
            $newImageName = $uploadResult['filename'];

            // Attempt isolated Events Edit WebP auto-optimization
            $optimizedResult = optimize_events_edit_image($uploadResult['filepath'], 1600, 1200, 82);
            if (
                ...
                creed_audit_log('UPLOAD_ACCEPTED', 'EVENT', $id, 'SUCCESS', [
    ...
    if (empty($error)) {
        ...
        if ($connect instanceof mysqli) {
            if ($newImageName !== null) {
                $deleteimage = '';
                $stmtOld = mysqli_prepare($connect, "SELECT `blog_image` FROM `events` WHERE `id` = ? LIMIT 1");
                if ($stmtOld) {
                ...
                $stmtUp = mysqli_prepare($connect, "UPDATE `events` SET `blog_image` = ?, `title` = ?, `detail` = ? WHERE `id` = ?");
                if ($stmtUp) {
                    mysqli_stmt_bind_param($stmtUp, "sssi", $newImageName, $title, $detail, $id);
                    if (mysqli_stmt_execute($stmtUp)) {
                        creed_audit_log('UPDATE', 'EVENT', $id, 'SUCCESS');
                        ...
                        creed_audit_log('UPDATE_FAILED', 'EVENT', $id, 'FAILURE', ['error' => mysqli_stmt_error($stmtUp)]);
                    }
                    ...
                    creed_audit_log('UPDATE_FAILED', 'EVENT', $id, 'FAILURE', ['error' => mysqli_error($connect)]);
                }
            } else {
                $stmtUp = mysqli_prepare($connect, "UPDATE `events` SET `title` = ?, `detail` = ? WHERE `id` = ?");
                if ($stmtUp) {
        ...
        header("Location: events.php?action=saved");
        exit;
    }
}
...
if ($connect instanceof mysqli && $id > 0) {
    $stmtFetch = mysqli_prepare($connect, "SELECT * FROM `events` WHERE `id` = ? LIMIT 1");
    if ($stmtFetch) {
...
<head>
  ...
  <title>Edit Event | Creed Tech Admin</title>
...
<body style="background:#F8FAFC;">
  ...
  <section style="margin-top: 100px;max-width:800px;margin-left:auto;margin-right:auto;padding:24px;background:#fff;border-radius:8px;border:1px solid #E2E8F0;">
    <h2 style="font-size:1.5rem;font-weight:700;color:#0F172A;margin-bottom:20px;">Edit Event #<?php echo e($id); ?></h2>
    <?php if (!empty($error)) : ?>
    ...
    <?php endif; ?>
    <form action="events_edit.php?id=<?php echo e($id); ?>" method="POST" enctype="multipart/form-data">
      <?php echo csrf_field(); ?>
      <div class="mb-3">
      ...
      <button class="btn btn-primary" type="submit" name="update_submit">Update Event</button>
